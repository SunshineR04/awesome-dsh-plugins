...
- [dsh-session-manager](https://github.com/SunshineR04/dsh-session-manager) — 已归档会话管理：设置页列出/恢复/彻底删除（直接物理删除，无备份层），会话菜单红色删除项、/sessions 命令与三个 agent 工具；已打开的会话也能立即删除（墓碑隐藏，重启后自动清理） ⭐0 · `dsh plugin add github:SunshineR04/dsh-session-manager`
